...
- **🔴 Live prod bug found while running the cleanup (now fixed):** both GDPR `SECURITY DEFINER` functions — `delete_user_account` (account deletion) and `export_user_data` (data export) — deleted/read from the dropped `public.trips` table, so **both were failing in production for every user** (`42P01 relation "public.trips" does not exist`). `export_user_data` also read `action_items` by a non-existent `profile_id`. Fixed + validated on staging; repo migration `070_fix_gdpr_functions_drop_trips.sql` (Lewis applies to prod). **Audit gap:** the code sweep scanned only `.from()` calls in TypeScript, not SQL function bodies — a follow-up should audit the remaining `SECURITY DEFINER` functions (`fn_import_batches`, `get_import_history`, `prediction_metrics_txn`) for the same class of dropped-table reference.
