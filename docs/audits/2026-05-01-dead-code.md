...
- Dependency dead weight: **0 removable** (per 2026-05-01-v2-audit.md §7 — `react-dom` peer-only, no truly orphan runtime deps)
...
2026-05-01-v2-audit.md §7 found **none**. Of 22 runtime dependencies, every one is either statically imported, dynamically imported (`html2canvas`), TS-resolved (`@types/papaparse`), or a framework peer (`react-dom`).
