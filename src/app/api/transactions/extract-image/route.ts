...
const extractionSchema = z.object({
  transactions: z.array(
    z.object({
      ...
      type: z
        .enum(['income', 'expense'])
        .describe('Whether money came in (income) or went out (expense)'),
    }),
  ),
  confidence: z
    .enum(['high', 'medium', 'low'])
    .describe(
      'How confident the extraction is — high means clear text, low means blurry or unclear',
    ),
})
...
- type: "income" if money came into the account, "expense" if money went out
...
If some transactions are readable but others aren't, include only the readable ones and set confidence to "medium".`,
    ...
    console.error('Image transaction extraction error:', err)
    return Response.json({ error: 'Failed to extract transactions from image.' }, { status: 500 })
  }
}
