...
import { evaluateActionItemReminders } from '@/lib/nudges/evaluators/action-item-reminder';
import { evaluateGoalMilestones } from '@/lib/nudges/evaluators/goal-milestone';
  ...
  for (const user of users) {
    try {
      await Promise.allSettled([
        evaluateActionItemReminders(sb, user.id),
        evaluateGoalMilestones(sb, user.id),
      ]);
    } catch (err) {
