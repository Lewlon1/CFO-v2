...
function buildGoalDeriveConfirmContext(
  ...
  const lines = [
    ...
    '',
    'The draft template below has a date slot — never fill it with an invented date. Once the user has named BOTH:',
    '- an amount (any sum reference: "£15k", "around $20,000", "the card debt"), AND',
    ...
    '',
    '[CTA:confirm_goal]Confirm[/CTA]',
    '',
    'The `[CTA:confirm_goal]Confirm[/CTA]` block is REQUIRED on a draft — emit it on its own line directly under the draft. It renders the tappable Confirm button; tapping it sends the user\'s confirmation back to you, at which point you call `create_goal`. The user can still type a correction instead of tapping.',
    '',
    'If both amount AND window are given, draft immediately. If only ONE is given, do NOT draft yet and do NOT invent the missing piece to fill the template — ask ONE follow-up question for exactly the piece that\'s missing (the amount, or the window), then draft on the next turn once you have both. You MUST NOT ask more than 2 clarifying questions before drafting — if a target window still hasn\'t landed after that, propose a sensible round date yourself when you draft, framed explicitly as a proposal ("say, 3 years out — change it if you have a date in mind") never as a fact you already know. Drafting is the forcing function — an imperfect goal that gets refined beats perfect data-gathering forever; inventing a fact is not the same as proposing a starting point.',
    '',
