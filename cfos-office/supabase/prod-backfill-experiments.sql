-- PROD BACKFILL — 052_experiment_engine
--
-- Apply by hand to production (iccelmjenljanqrhhzdv). Lewis ONLY.
-- Do NOT run via Claude Code or any automated runner.
--
-- Procedure:
--   1. Run inside a transaction (BEGIN ... COMMIT/ROLLBACK).
--   2. After the DDL block, run the verification queries at the bottom.
--   3. If verification passes, COMMIT. Otherwise ROLLBACK.
--
-- Notes:
--   * The status UPDATEs map old enum values to the new lifecycle vocabulary.
--     On any prod env with rows in 'dismissed' / 'completed' they will rewrite
--     status before the CHECK constraint is replaced.
--   * goals.type is backfilled by keyword heuristic. After commit, spot-check
--     that high-traffic goals map sensibly; correct outliers manually.
...
alter table public.proposed_experiments
  ...
  check (status in (
    'proposed','accepted','active','succeeded','partial','failed','expired','declined'
  ));

-- ---------------------------------------------------------------------------
-- 2. New columns on proposed_experiments.
-- ---------------------------------------------------------------------------
alter table public.proposed_experiments
...
-- ---------------------------------------------------------------------------
-- 5. RLS — filter soft-deleted; allow self-updates.
-- ---------------------------------------------------------------------------
...
-- ===========================================================================
-- Verification (run these before COMMIT):
-- ===========================================================================
-- a) Status distribution sane?
--    select status, count(*) from public.proposed_experiments group by status;
--
-- b) New columns visible?
--    select column_name from information_schema.columns
--      where table_schema='public' and table_name='proposed_experiments'
--        and column_name in ('template_id','starts_at','ends_at','target_metric',
--                            'scoring_breakdown','outcome_self_report');
--
-- c) goals.type populated for every row?
--    select count(*) filter (where type is null) as nulls,
--           count(*) as total
--      from public.goals;
--
-- d) Distribution sensible?
--    select type, count(*) from public.goals group by type;
--
