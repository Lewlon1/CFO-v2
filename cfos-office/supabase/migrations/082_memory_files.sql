-- 082_memory_files.sql
--
-- The Filing Cabinet — per-user markdown memory the CFO reads and writes on
-- demand, organised into the four folders the user already sees in the office
-- UI (Goals, Cash Flow, Values & You, Net Worth).
--
-- Why: today every scrap of user knowledge is dumped into the system prompt on
-- every chat turn (unbounded financial_portrait rows included). That is
-- expensive and it still forgets. These tables back a compact prompt *index*
-- (one line per file) plus a read_memory_file tool that fetches bodies
-- mid-turn.
--
-- THE MEMORY IS THE VISIBLE FILING CABINET. A later phase lets users open and
-- edit these exact rows in the office UI. That is why:
--   * content is markdown prose, not JSON;
--   * user_edited_at is a one-way stamp — once a user has touched a file their
--     words are authoritative and the CFO may only APPEND to it, never replace
--     it (enforced in src/lib/memory/files.ts, `mode: 'replace'`);
--   * memory_file_revisions keeps the PRE-change snapshot of every CFO
--     overwrite so nothing the model replaces is lost.
--
-- ⚠️ CLOBBER HAZARD — READ BEFORE EDITING THE GDPR RPCs BELOW.
-- This migration CREATE OR REPLACEs public.export_user_data(uuid) and
-- public.delete_user_account(uuid) purely to teach them about the two new
-- tables. Both are SECURITY DEFINER (they run as owner, bypassing RLS) and take
-- a user id as an argument. export_user_data's authorization guard has ALREADY
-- been silently dropped once by a CREATE OR REPLACE that omitted it (see the
-- headers of 073_secure_export_user_data.sql and 077_lockdown_user_data_rpcs.sql)
-- — that was a full RLS bypass / GDPR data breach. Both bodies below are copied
-- from the CURRENT LATEST definitions in this repo (export_user_data from 073,
-- delete_user_account from 070) and preserve, byte-for-byte:
--   * the in-function guard (trust service_role; otherwise auth.uid() must
--     match p_user_id, else raise 42501);
--   * SET search_path TO 'public';
--   * export_user_data's REVOKE/GRANT lock-down, re-issued below.
-- ANY future migration that recreates either function MUST carry all three
-- forward. Start from the latest definition in the repo, never an older one.
--
