-- 074_onboarding_estimates.sql
--
-- OB-1 — Estimates-first onboarding. New table `onboarding_estimates`,
-- one row per user, holding:
--   * the raw band taps the user makes while sketching their month BEFORE
--     any statement upload (housing/subs/bills/food-out/save-reach),
--   * income + top value + per-domain verdicts,
--   * the deterministic derive() engine output (`derived`, incl. engine_version),
--   * per-band verification states written later, once a real statement is
--     checked against the estimates.
--
-- Band columns store band IDs only — the midpoints/ranges those IDs map to
-- live in the versioned TS engine, never in the DB, so re-banding never
-- requires a migration. Same for verdict/top-value vocabularies: no CHECK
-- constraints on them by design.
--
-- GDPR notes (this migration also amends both SECURITY DEFINER functions):
--   * delete_user_account() — re-created below with an explicit
--     onboarding_estimates delete. The function deletes per-table (it does
--     not rely on cascades), so the new table must be enumerated. The
--     ON DELETE CASCADE on user_id is belt-and-braces for direct
--     user_profiles row deletes (e.g. the Playwright harness teardown).
--   * export_user_data() — re-created with onboarding_estimates included so
--     Article 20 exports stay complete (the table is user financial data),
--     AND with the caller-authorization guard its sibling
--     delete_user_account already carries (service_role bypass; otherwise
--     auth.uid() must be non-null and match p_user_id). The 070 body is
--     preserved byte-for-byte EXCEPT those two additions: the new guard
--     block and the new onboarding_estimates block.
--   * Both GDPR functions get an explicit REVOKE/GRANT block at the end of
--     this file (062 precedent): EXECUTE revoked from PUBLIC, anon and
--     authenticated; granted to service_role only. The in-function guards
--     are defense-in-depth behind that revoke.
--   * There is no anonymise function in the schema (the anonymised_at
--     columns from 028 are read by markOnboardingCompleteIfReady, never
--     set) — nothing to amend on that path. If one is ever added, it must
--     delete the onboarding_estimates row outright.
--
-- RLS: SELECT/INSERT/UPDATE policies only — a DELETE policy is deliberately
-- omitted. Row removal happens via delete_user_account() (service role) or
-- the user_profiles ON DELETE CASCADE; users never delete this row directly.
