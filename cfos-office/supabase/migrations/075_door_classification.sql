-- 075_door_classification.sql
--
-- OB-1 — Door classification + composite persona columns on user_profiles.
-- The "door" is what brings the user into the office, classified into an
-- internal 4-family taxonomy (growth / security / agency / candor). Family
-- names and the raw confidence score are INTERNAL ONLY — never shown in any
-- UI, never spoken by the CFO.
--
-- The door raw text deliberately reuses the existing
-- user_profiles.entry_struggle_text column (040) — no duplicate column.
--
-- GDPR: door_reflection and composite_truer_line contain user-typed text
-- (entry_struggle_text already did). All user_profiles columns are removed
-- by the hard-delete path (delete_user_account deletes the whole row).
-- There is no anonymise function in the schema to amend (the anonymised_at
-- columns from 028 are read, never set); if one is ever added, its
-- user_profiles scrub list must include entry_struggle_text,
-- door_reflection and composite_truer_line.
