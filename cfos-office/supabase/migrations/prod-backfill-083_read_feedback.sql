-- prod-backfill-083_read_feedback.sql
--
-- ⚠️ PRODUCTION-ONLY — DO NOT APPLY VIA AUTOMATION. ⚠️
-- Lewis runs this by hand against prod (iccelmjenljanqrhhzdv).
-- The staging twin is supabase/migrations/083_read_feedback.sql
-- (qlbhvlssksnrhsleadzn), applied through the normal path. The SQL body below
-- is byte-identical to that file; only this header differs.
--
-- WHAT: adds public.read_feedback — user-reported errors in a First Read, keyed
-- to the same first_read_message_id wow_assessments uses — and CREATE OR
-- REPLACEs the two GDPR RPCs so the new table is exported and deleted.
--
-- ADDITIVE: yes. Creates one table, two indexes, two RLS policies. Replaces two
-- function bodies (copied from 082 and extended; no other behaviour changes).
...
--
-- PRE-RUN CHECKS (expect: 0 rows, then the 082-era function definitions):
--   select count(*) from information_schema.tables
--    where table_schema = 'public' and table_name = 'read_feedback';        -- expect 0
--   select proname from pg_proc
--    where proname in ('export_user_data', 'delete_user_account');          -- expect both
--
-- POST-RUN CHECKS:
--   select count(*) from information_schema.tables
--    where table_schema = 'public' and table_name = 'read_feedback';        -- expect 1
--   select policyname, cmd from pg_policies
--    where tablename = 'read_feedback' order by policyname;
--     -- expect exactly: read_feedback_insert_own (INSERT),
--     --                 read_feedback_select_own (SELECT)
--   select relrowsecurity from pg_class
--    where oid = 'public.read_feedback'::regclass;                          -- expect true
--   -- Proves the RPC replace did not drop a table from the export:
--   select jsonb_object_keys(public.export_user_data('<a real prod user uuid>'))
--    intersect select unnest(array['read_feedback','memory_files','conversations']);
--     -- expect all three
